...
ApplicationWindow {
    ...
    function debianInstallSystemDeps(){
        ...
        case "distroubuntu":
            ...
            else if(ubuntuversion === "ubuntucosmic"){
                var installubuntusysdepargcosmic = ["-c", "pkexec /home/$USER/mycroft-core/ubuntuCosmic.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installubuntusysdepargcosmic);
                mainsession.startShellProgram();
                currentPos = "debianinstallsysdepscompleted"
            }
            else if(ubuntuversion === "ubuntubionic"){
                var installubuntusysdepargdisco = ["-c", "pkexec /home/$USER/mycroft-core/ubuntuDisco.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installubuntusysdepargdisco);
                mainsession.startShellProgram();
                currentPos = "debianinstallsysdepscompleted"
            }
            break
    ...
    function debianInstallPlasmoidDeps(){
        ...
        case "distroubuntu":
            if(ubuntuversion === "ubuntuartful"){
                ...
                var installplasdepscosmic = ["-c", "pkexec /tmp/installers/kde_plasmoid_ubuntu/appletv2depscosmic.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installplasdepscosmic);
                mainsession.startShellProgram();
                currentPos = "debianinstallplasmoiddepscompleted"
            }
            else if(ubuntuversion === "ubuntudisco"){
                var installplasdepsdisco = ["-c", "pkexec /tmp/installers/kde_plasmoid_ubuntu/appletv2depsdisco.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installplasdepsdisco);
                mainsession.startShellProgram();
                currentPos = "debianinstallplasmoiddepscompleted"
            }
            break
    ...
    function debianInstallMycroftGUIDeps(){
        mainsession.hasFinished = false
        currentPos = ""
        progressText.text = "Installing Mycroft-GUI System Dependencies"

        switch(distrotype){
        case "distroubuntu":
            if(ubuntuversion === "ubuntuartful"){
                var installguidepsart = ["-c", "pkexec /tmp/installers/kde_plasmoid_ubuntu/guidepsart.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installguidepsart);
                mainsession.startShellProgram();
                currentPos = "debianinstallguidepscompleted"
            }
            else if(ubuntuversion === "ubuntubionic"){
                var installguidepsbio = ["-c", "pkexec /tmp/installers/kde_plasmoid_ubuntu/guidepsbio.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installguidepsbio);
                mainsession.startShellProgram();
                currentPos = "debianinstallguidepscompleted"
            }
            else if(ubuntuversion === "ubuntucosmic"){
                var installguidepscosmic = ["-c", "pkexec /tmp/installers/kde_plasmoid_ubuntu/guidepscosmic.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installguidepscosmic);
                mainsession.startShellProgram();
                currentPos = "debianinstallguidepscompleted"
            }
            else if(ubuntuversion === "ubuntudisco"){
                var installguidepsdisco = ["-c", "pkexec /tmp/installers/kde_plasmoid_ubuntu/guidepsdisco.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installguidepsdisco);
                mainsession.startShellProgram();
                currentPos = "debianinstallguidepscompleted"
            }
            break
        case "distrodebian":
            if(debianversion === "debianunstable"){
                var installguidepsdebu = ["-c", "pkexec /tmp/installers/kde_plasmoid_debian/guidepsdebu.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installguidepsdebu);
                mainsession.startShellProgram();
                currentPos = "debianinstallguidepscompleted"
            }
            break
        case "distroneon":
            if(neonversion === "neonxenial"){
                ...
                var installguidepsneonb = ["-c", "pkexec /tmp/installers/kde_plasmoid_neon/guidepsneonb.sh"]
                mainsession.setShellProgram("bash");
                mainsession.setArgs(installguidepsneonb);
                mainsession.startShellProgram();
                currentPos = "debianinstallguidepscompleted"
            }
            break
    ...
    function debianInstallMycroftGUI(){
        mainsession.hasFinished = false
        currentPos = ""
        progressText.text = "Building & Installing Mycroft-GUI"
        var installplasmoid = ["-c", "pkexec /tmp/installers/kde_plasmoid_debian/installGUI.sh"]
        mainsession.setShellProgram("bash");
        mainsession.setArgs(installplasmoid);
        mainsession.startShellProgram();
    ...
    function debianInstallQMLLottie(){
        mainsession.hasFinished = false
        currentPos = ""
        progressText.text = "Building & Installing QML Lottie"
        var installplasmoid = ["-c", "pkexec /tmp/installers/kde_plasmoid_debian/installLottie.sh"]
        mainsession.setShellProgram("bash");
        mainsession.setArgs(installplasmoid);
        mainsession.startShellProgram();
        currentPos = "debianlottieinstallcompleted"
    }
    ...
    //            currentPos = "fedorainstallsysdepscompleted"
    //        }
    //        else if(fedoraversion === "fedora29"){
    //            var installsysdeparg3 = ["-c", "pkexec /home/$USER/mycroft-core/depsFed29.sh"]
    //            mainsession.setShellProgram("bash");
    //            mainsession.setArgs(installsysdeparg3);
    //            mainsession.startShellProgram();
    //            currentPos = "fedorainstallsysdepscompleted"
    //        }
    //        else if(fedoraversion === "fedora30"){
    //            var installsysdeparg4 = ["-c", "pkexec /home/$USER/mycroft-core/depsFed30.sh"]
    //            mainsession.setShellProgram("bash");
    //            mainsession.setArgs(installsysdeparg4);
    //            mainsession.startShellProgram();
    //            currentPos = "fedorainstallsysdepscompleted"
    //        }
    //    }
                                    ...
                                    session: QMLTermSession{
                                        ...
                                        onFinished: {
                                            ...
                                            case "debiangetlottiecompleted":
                                                ...
                                                break;
                                            case "debianlottieinstallcompleted":
                                                hasFinished = true;
